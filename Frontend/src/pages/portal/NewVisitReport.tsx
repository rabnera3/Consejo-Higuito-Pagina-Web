...
export default function NewVisitReportPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await api.post('/visit-reports', payload);

            if (response.success) {
                navigate('/portal/reportes-visita');
            } else {
                setError(response.message || 'Error al crear el reporte');
            }
        } catch (err: any) {
    ...
    return (
        <section className="cih-card">
            <div className="cih-card__body">
                <h2 className="cih-card__title">Nuevo Reporte de Visita</h2>
                <p className="cih-card__subtitle">Documenta hallazgos y acuerdos de la visita a campo.</p>
